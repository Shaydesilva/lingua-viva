...
`Learner: ${model?.narrative || 'New learner'} About him: ${profile?.goals || 'Lives in Rio'} Last session mood: ${lastAnalysis.mood} Topics: ${JSON.stringify(lastAnalysis.topics)} Comp: ${comp.toFixed(0)}, Prod: ${prod.toFixed(0)} Phase: ${profile?.current_phase_name}, Words known: ${profile?.words_known} Review due: ${(reviewWords || []).map(w => w.word).join(', ')} New available: ${newAvailable.slice(0, 6).map(w => `${w.word}(${w.translation})`).join(’, ’)}
...
const sessionsText = sessions.map((s, i) => `Session ${sessionCount - i}: Summary: ${s.session_summary} Mood: ${s.user_mood} Topics: ${JSON.stringify(s.topics_discussed)} Duration: ${Math.round(s.duration_seconds / 60)}min Comprehension hits: ${(s.comprehension_data || []).filter(c => c.understood).length}/${(s.comprehension_data || []).length} Production: ${(s.production_words || []).join(', ') || 'none'} Errors: ${(s.accuracy_data || []).filter(a => !a.correct).map(a => a.attempt).join(', ') || 'none'}`
).join(’\n\n’);
